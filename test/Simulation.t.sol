...
contract PoolSimulationTest is Test {
    ...
    function calculateMarketCapEth(uint256 price, uint256 circulatingSupply, uint256 virtualTokenReserve)
        internal
        pure
        returns (uint256)
    {
    ...
    function test_marketSimulation() public {
        ...
        console.log("Tokens in Contract:", Formatting.formatTokens(vm, lotry.balanceOf(address(lotry))));
        console.log("ETH Raised in Contract:", Formatting.formatEther(vm, lotry.ethRaised(), 4));
        console.log("Initial Token Price (ETH):", Formatting.formatSmallPrice(vm, lotry.calculateCurrentPrice()));
        uint256 initialMarketCapInEth = calculateMarketCapEth(
            lotry.calculateCurrentPrice(), INITIAL_SUPPLY - lotry.balanceOf(address(lotry)), vTokenReserve
        );
        console.log("Initial Market Cap (ETH):", Formatting.formatEther(vm, initialMarketCapInEth, 4));
        console.log(
            "Initial Market Cap (USD):", Formatting.formatUsd(vm, (initialMarketCapInEth * ethToUsdRate) / 1e18)
        );
        console.log("------------------------------------");
        ...
        for (uint256 i = 0; i < 50 && tokensSoldTotal < targetTokensSold; i++) {
            vm.prank(buyer1);
            ...
            try lotry.buy{value: buyAmountWei}() {
                ...
                uint256 marketCapInEth = calculateMarketCapEth(currentPrice, circulatingSupply, vTokenReserve);
                ...
                console.log("Buy failed, likely due to insufficient reserves. Stopping loop.");
                break;
            }
        ...
        console.log("Accumulated Pool Fee (ETH):", Formatting.formatEther(vm, lotry.accumulatedPoolFee(), 6));
        console.log("------------------------------------");
        ...
        for (uint256 i = 0; i < sellPercentages.length; i++) {
    ...
    function test_priceIncreaseSimulation50Buys() public {
        ...
        for (uint256 i = 0; i < numberOfBuys; i++) {
            buyAmounts[i] = minBuyAmount + ((maxBuyAmount - minBuyAmount) * i) / (numberOfBuys - 1);
        }
        ...
        for (uint256 i = 0; i < numberOfBuys; i++) {
            ...
            uint256 tokensReceived = lotry.balanceOf(currentBuyer) - tokenBalanceBefore;
            ...
            uint256 marketCapInEth = calculateMarketCapEth(
                priceAfter, lotry.INITIAL_SUPPLY() - lotry.balanceOf(address(lotry)), lotry.virtualTokenReserve()
            );
            uint256 marketCapUsd = (marketCapInEth * ethToUsdRate) / 1e18;
        ...
        for (uint256 i = 0; i < numberOfBuys; i++) {
            ...
            if (lotry.balanceOf(currentBuyer) > 0) {
                ...
                int256 profitLoss = int256(ethReceived) - int256(buyAmounts[i]);
    ...
    function test_tokenDepletionSimulation() public {
        ...
        for (uint256 i = 0; i < 100 && tokensSoldTotal < targetTokens; i++) {
        ...
        console.log("Total ETH required (USD):", Formatting.formatUsd(vm, (totalEthSpent * ethToUsdRate) / 1e18));
        console.log("Total buys executed:", vm.toString(buyCount));
        console.log("Tokens sold:", Formatting.formatTokens(vm, tokensSoldTotal));
        console.log("Tokens remaining:", Formatting.formatTokens(vm, lotry.balanceOf(address(lotry))));
        console.log("Final price:", Formatting.formatSmallPrice(vm, lotry.calculateCurrentPrice()));
        console.log("Price increase:", string.concat(vm.toString(lotry.calculateCurrentPrice() / initialPrice), "x"));
    ...
    function test_mixedBuysAndSells() public {
        ...
        actions[0] = Action({actionType: ActionType.Buy, trader: buyer1, amount: 1 ether});
        actions[1] = Action({actionType: ActionType.Buy, trader: buyer2, amount: 2.5 ether});
        actions[2] = Action({actionType: ActionType.Sell, trader: buyer1, amount: 50}); // 50%
        actions[3] = Action({actionType: ActionType.Buy, trader: buyer3, amount: 0.75 ether});
        actions[4] = Action({actionType: ActionType.Sell, trader: buyer2, amount: 25}); // 25%
        actions[5] = Action({actionType: ActionType.Buy, trader: buyer1, amount: 3 ether});
        actions[6] = Action({actionType: ActionType.Buy, trader: buyer2, amount: 1.2 ether});
        actions[7] = Action({actionType: ActionType.Sell, trader: buyer3, amount: 100}); // 100%
        actions[8] = Action({actionType: ActionType.Sell, trader: buyer1, amount: 30}); // 30%
        actions[9] = Action({actionType: ActionType.Buy, trader: buyer2, amount: 5 ether});
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action.actionType == ActionType.Buy) {
                ...
                uint256 tokensReceived = lotry.balanceOf(action.trader) - tokenBalanceBefore;
                uint256 priceAfter = lotry.calculateCurrentPrice();
                int256 priceChange = int256(priceAfter) - int256(priceBefore);
                ...
                if (traderBalance > 0) {
                    ...
                    lotry.sell(sellAmount);
                    uint256 ethReceived = action.trader.balance - ethBalanceBefore;
                    uint256 priceAfter = lotry.calculateCurrentPrice();
                    int256 priceChange = int256(priceAfter) - int256(priceBefore);
                    int256 priceChangePercent = int256(priceChange * 100) / int256(priceBefore);
                    uint256 marketCapInEth = calculateMarketCapEth(
